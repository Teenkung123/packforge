...
| Direct build ownership | Public graph is direct-only; 53 leaves authoritative; direct contract AST/self-test and task PASS; registry dry-run PASS. Fabric native preprocessing has focused proof for archive capture on 16 targets, SharedZip access on 17 targets (1.20.2 through 1.21.11), and Bitmap provider definition on all 19 targets (1.20.1 through 26.1); mc26 retains physical seams where needed. Nested Gradle remains parity oracle. |
| Source metrics | 213 production Java files; 16,430 LOC; 106 bridge files / 5,410 LOC (32.93%); zero exact/normalized duplicate groups; 15 target/version conditional lines; five Stonecutter conditional blocks (maximum 40 lines); three renderer bodies; two adapters. |
