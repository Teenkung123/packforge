...
| **I — compatibility-profile harness** | `PARTIAL` | The schema-2 catalog represents all 30 isolated and six high-risk recipes; materializer, hash cache, provenance, fixtures, and three loader transports exist. Every generated fixture now carries the semantic-hash marker required by controlled runtime evidence. | 21 profiles are `AVAILABLE`/`UNTESTED`, 12 `PENDING_METADATA`/`UNTESTED`, three `UNAVAILABLE`, and zero PASS. Some pending reasons require retargeting or evidence-backed `UNAVAILABLE`; the default-on Quick Pack profile has no promoted feature set. | Resolve every pending disposition, retain immutable input/evidence hashes, add any missing path instrumentation, and execute all available isolated/high-risk recipes. |
