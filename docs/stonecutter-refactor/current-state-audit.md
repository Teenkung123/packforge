...
| Direct build ownership | Public graph is direct-only; 53 leaves authoritative; direct contract AST/self-test and task PASS; registry dry-run PASS. Fabric native preprocessing has focused proof for archive capture on 16 targets, SharedZip access on 17 targets (1.20.2 through 1.21.11), ReloadableResourceManager on 18 pre26 targets (1.20.1 through 1.21.11), RuntimeResourceHash on 17 targets (1.20.1 through 1.21.10), and Bitmap provider definition on all 19 targets (1.20.1 through 26.1); later physical seams remain where needed. Nested Gradle remains parity oracle. |
| Source metrics | 215 production Java files; 16,496 LOC; 108 bridge files / 5,476 LOC (33.20%); zero exact/normalized duplicate groups; 15 target/version conditional lines; seven Stonecutter conditional blocks (maximum 40 lines); three renderer bodies; two adapters. |
