...
| **E - Authoritative direct Stonecutter build** | `PARTIAL` | `stonecutter-build.gradle` and root `build.gradle` use `Exec` to call nested platform wrappers. Public build tasks aggregate standalone builds instead of one direct Stonecutter graph. `buildStonecutterAll` and `verifyStonecutterAll` are absent. | Current structural checks pass only for nested outputs. | Migrate registry-derived target/loader nodes to direct loader plugin tasks, prove Java 17/21/25 and remap/JarJar parity, then remove delegation. |
...
1. `settings.gradle` manually defines Stonecutter nodes and `stonecutter-build.gradle` invokes nested wrappers, so the authoritative-direct-build claim is false.
...
3. Complete D/E direct-build migration, then measure and finish F/G.
