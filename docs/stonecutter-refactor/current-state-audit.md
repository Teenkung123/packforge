...
- Validation stays proportional: the 2026-08-16 checkpoint includes the requested clean/full build and all-53 parity oracle; no broad runtime matrix is implied.
...
| **E — authoritative direct build** | `IMPLEMENTED_UNVERIFIED` | Public tasks use 53 direct leaves and registry toolchains. `stonecutter-build.gradle` now validates node context only; `packforge-stonecutter-direct.gradle` registers the always-on direct leaf, while `packforge-stonecutter-legacy-parity.gradle` is loaded only by explicit parity opt-in. Focused Fabric seams remain proven at their bounded boundaries. | Direct-contract self-test, `buildStonecutterAll`, `verifyStonecutterAll`, and opt-in all-53 `verifyStonecutterParityAll` pass. Clean `buildAllSupported` and aggregate artifact verification also pass. Runtime proof remains open. | Keep the optional legacy oracle until current runtime evidence justifies removing it; then rerun the direct graph gates. |
| **F — source ownership and reduction** | `IMPLEMENTED_UNVERIFIED` | Metrics report 209 files, 15,814 LOC, zero duplicate groups, six target/version conditional lines, zero platform target-key conditional lines, and exact-version bridge LOC below version-common LOC. The two Forge mod-list files are thin API-family bridges. | The report records a current-tree 2,347-LOC added ledger and a 3,370-LOC adjusted reduction against 3,368 required; the executable metric gate passes. Behavior parity and runtime ownership are not exhaustive. | Keep Phase F IMPLEMENTED_UNVERIFIED until behavior parity/runtime evidence is complete. |
...
- The all-53 opt-in parity oracle, clean publication build, structural artifact checks, size inspection, and release-manifest verification passed. The 62-cell runtime matrix, third-party profile, and release publication remain unrun.
...
1. Nested Gradle still exists as an explicit parity oracle; its all-53 comparison now passes, but the direct helper should remain until current runtime evidence and broader preprocessing ownership are complete.
...
1. Finish Phase E capability migration and current runtime proof; delete the optional legacy parity path only after direct parity and runtime evidence remain reproducible.
