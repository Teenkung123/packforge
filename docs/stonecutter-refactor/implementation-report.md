...
|---|---|---|---|
| A — audit and source gates | `PARTIAL` | Deterministic source inventory, duplicate checks, ownership/category metrics, conditional counts, renderer-family counts, registry/publication counts, and the canonical A-L audit exist. | Run a current authoritative build and artifact verification after later direct-build changes. |
...
| G — configuration renderers | `IMPLEMENTED_UNVERIFIED` | Unknown fields survive save; integer validation is shared; renderer families reduced to three bodies plus two adapters; atlas retry configured/effective state is separated. | Existing-file replacement-failure coverage plus live UI parity across entry routes/renderer families and shader/atlas runtime behavior. |
