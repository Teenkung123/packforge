...
- Status: `PHASE_E_PARTIAL`.
- Scope: canonical guarded Fabric RuntimeResourceHash for exact 1.20.1 through 1.21.10 (17 targets); retained physical seams are raw Predicate/String in 1.21.11 and Identifier in mc26. Standalone, Forge, and NeoForge remain untouched.
...
## 2026-08-16 — clean build and all-target verification

- Scope: verify the current `1.4` branch after the cross-loader slim-MixinExtras and source-reduction work, including a clean build, every direct Stonecutter node, opt-in legacy parity, final artifacts, size report, and release manifest.
- Reproducibility repair: `platform/neoforge/build.gradle` pins only the transitive `org.apache.logging.log4j:log4j-api/core:2.11.+` request from NeoForge 20.6's historical `net.minecraftforge:unsafe` graph to `2.11.2`; NeoForged Releases returned an empty metadata document for that dynamic range. NeoForge's declared 2.22.1 dependency remains unchanged. The focused `:neoforge:mc1_20_6:createMinecraftArtifacts` task passed afterward.
- Build evidence: `clean buildAllSupported` PASS in 7m07s (415 tasks; 248 executed); exactly 20 current version-1.4 publication JARs were produced. `buildStonecutterAll` PASS (713 tasks), `verifyStonecutterAll` PASS (819 tasks), and opt-in `verifyStonecutterParityAll -Ppackforge_enable_stonecutter_parity=true` PASS across all 53 direct/legacy comparisons in 28m35s.
- Default entrypoint evidence: root `build` PASS in 10m18s (796 tasks; 382 executed) across the configured Java toolchain workers; the final root directory still contains exactly 20 JARs and the post-build `verifyExistingArtifacts` check PASS.
- Artifact evidence: `verifyAllArtifacts`, `verifyExistingArtifacts`, and `inspectArtifactSizes` PASS. All 20 final JARs contain exactly one loader-specific MixinExtras `0.5.4:slim` nested artifact, the optimized 128x128/10,607-byte icon, required mixin/refmap metadata, and no duplicate ZIP entries.
- Manifest evidence: `python scripts/Generate-ReleaseManifest.py --artifacts-dir build/libs --output-dir build/libs/release` generated the 20-entry manifest; `verifyReleaseManifest` PASS verified registry metadata and SHA-256/SHA-512 artifact hashes.
- Limits: no 62-cell Minecraft runtime matrix, same-binary range proof, third-party profile execution, cancellation scenario, or release publication was run. Gradle/Stonecutter/deprecation/unsafe-operation messages were warnings only; no build crash or compile/package failure remained.
