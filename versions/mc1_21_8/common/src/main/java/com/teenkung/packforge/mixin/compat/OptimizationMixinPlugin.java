...
/** Selects lower-target ownership before overlapping mixins transform their targets. */
public class OptimizationMixinPlugin implements IMixinConfigPlugin {
	private OptimizationCompatibility.State compatibility;
	@Override public void onLoad(String mixinPackage) { compatibility = EarlyModDiscovery.capture(); }
	@Override public boolean shouldApplyMixin(String targetClassName, String mixinClassName) {
		if (compatibility == null) throw new IllegalStateException("PackForge mixin ownership requested before loader discovery");
		return MixinCompatibility.shouldApply(mixinClassName, compatibility);
	}
