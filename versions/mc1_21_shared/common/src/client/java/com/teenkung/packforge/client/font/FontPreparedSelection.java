...
public final class FontPreparedSelection implements AutoCloseable {
	...
	private static FontPreparedSelection computeOwned(List<GlyphProvider.Conditional> providers,
		Set<FontOption> options, PreparationBudget.Reservation allocation) {
		...
		supportedGlyphs.forEach(codepoint -> {
			for (GlyphProvider provider : selectedProviders) {
				...
				if (glyph == SpecialGlyphs.MISSING) break;
				glyphsByWidth.computeIfAbsent(Mth.ceil(glyph.getAdvance(false)),
