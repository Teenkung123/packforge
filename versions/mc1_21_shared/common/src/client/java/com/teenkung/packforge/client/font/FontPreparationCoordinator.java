...
public final class FontPreparationCoordinator implements AutoCloseable {
	...
	private static final int MAX_UNICODE_CODEPOINTS = 0x110000;
	private static final long TTF_SUPPORTED_SET_BYTES = 32L * 1024L * 1024L;
	...
	private FontPreparationCoordinator(PreparationBudget.Scope budget, ReloadExecutionContext context,
		Set<FontOption> options, boolean diagnostics) {
	...
	public boolean finalizeProviders(List<GlyphProvider.Conditional> original,
		GlyphProvider.Conditional fallback) {
	...
	private boolean finalizeReserved(List<GlyphProvider.Conditional> original,
		GlyphProvider.Conditional fallback) {
		...
		if (entry != null && entry.owner == Thread.currentThread() && !entry.result.isDone()) {
			entry.release();
			return false;
		}
		if (entry == null) {
	...
	public FontPreparedSelection selection(List<GlyphProvider.Conditional> providers,
	...
	private int supportedCount(GlyphProvider provider) {
		...
		if (fixtureCounts) return provider.getSupportedGlyphs().size();
		int exact = supportedCountWithoutAllocation(provider);
		if (exact >= 0) return exact;
		// 1.21.1's TTF provider scans FreeType into a new IntOpenHashSet on every
		// call. Keep that worker-side generic path bounded before invoking it.
		if (provider.getClass() != TrueTypeGlyphProvider.class) return -1;
		PreparationBudget.Reservation probe = budget.tryReserve(TTF_SUPPORTED_SET_BYTES);
		if (probe == null) return -1;
		try {
			int count = provider.getSupportedGlyphs().size();
			return count <= MAX_UNICODE_CODEPOINTS ? count : -1;
		} finally { probe.close(); }
	}
	...
	static int supportedCountWithoutAllocation(GlyphProvider provider) {
		...
		if (!(provider instanceof FontProviderGlyphMapAccess accessor)) return -1;
		int[] count = {0};
	...
	private FontSelectionSummary compute(List<GlyphProvider.Conditional> providers) {
		...
		FontSelectionSummary summary = new FontSelectionSummary(ordered.winningSlots, ordered.selectedSlots,
			buckets, System.nanoTime() - started);
	...
	private OrderedGlyphs prepareOrdered(List<GlyphProvider.Conditional> providers) {
		...
		for (int slot = 0; slot < providers.size(); slot++) {
			...
			if (provider.getClass() == AllMissingGlyphProvider.class) {
				...
				for (int codepoint : supported) setSelectionIfAbsent(plans, codepoint,
					firstMissing < 0 ? slot : firstMissing);
			}
		}
		...
		for (int codepoint : warmUnion) {
			...
			if (result.glyph != SpecialGlyphs.MISSING) {
				plans.put(codepoint, pair(Mth.ceil(result.glyph.getAdvance(false)), result.slot));
			}
		}
	...
	private record OrderedGlyphs(int[] codepoints, int[] widths, int size,
		List<Integer> winningSlots, List<Integer> selectedSlots) {}
	...
	private static void resolve(List<GlyphProvider.Conditional> providers, int codepoint, int first,
		...
		for (int slot = first; slot < providers.size(); slot++) {
			...
			GlyphInfo glyph = providers.get(slot).provider().getGlyph(codepoint);
			result.queries++;
	...
	static boolean known(List<GlyphProvider.Conditional> providers) {
		// 1.21.1 exposes only the TTF provider's skip set; supportedCount() admits
		// its worker-side generic scan, while 1.21.4/1.21.8 use target accessors.
	...
	synchronized SpaceAdvanceCodec.Outcome admitSpaceCodec() {
		...
		return closed || budget == null || budget.isRetired()
			? SpaceAdvanceCodec.Outcome.STALE : SpaceAdvanceCodec.Outcome.BUDGET;
	}
	...
	@Override public void close() {
		...
		synchronized (this) {
			if (closed) return;
			closed = true;
			retired = new ArrayList<>(entries.values());
			entries = Map.of();
		}
	...
	private static final class StackKey {
		...
		@Override public boolean equals(Object other) {
			...
			for (int index = 0; index < identities.length; index++) {
				if (identities[index] != key.identities[index]) return false;
			}
			return true;
		}
	}
