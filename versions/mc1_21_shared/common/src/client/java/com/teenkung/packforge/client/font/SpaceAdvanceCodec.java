...
public final class SpaceAdvanceCodec extends MapCodec<Map<Integer, Float>> {
	...
	private static final Class<?> VANILLA_FIELD_TYPE = Codec.unboundedMap(ExtraCodecs.CODEPOINT, Codec.FLOAT)
		.fieldOf("advances").getClass();
	private static final String VANILLA_FIELD_DESCRIPTION = Codec.unboundedMap(ExtraCodecs.CODEPOINT, Codec.FLOAT)
		.fieldOf("advances").toString();
	...
	static MapCodec<Map<Integer, Float>> wrap(UnboundedMapCodec<Integer, Float> source, String field,
		...
		if (!"advances".equals(field) || source.keyCodec() != ExtraCodecs.CODEPOINT
			|| source.elementCodec() != Codec.FLOAT || source.getClass() != UnboundedMapCodec.class
			|| original.getClass() != VANILLA_FIELD_TYPE || !VANILLA_FIELD_DESCRIPTION.equals(original.toString())) {
			return original;
		}
	...
	@Override public <T> DataResult<Map<Integer, Float>> decode(DynamicOps<T> ops, MapLike<T> input) {
		...
		if (context == null || coordinator == null || !context.features().fontPrepareProviderSelectionEnabled()) {
		...
		if (ops != JsonOps.INSTANCE || input.getClass() != JSON_MAP_TYPE) {
			return fallback(ops, input, coordinator, 0, Outcome.INPUT);
		}
		...
		// hash-builder nodes, parse temporaries and bookkeeping until that builder leaves scope;
		// retain no extra map or reference after successful decoding.
