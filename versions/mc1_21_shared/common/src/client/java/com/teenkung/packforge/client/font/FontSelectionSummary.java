...
record FontSelectionSummary(List<Integer> winningSlots, List<Integer> selectedSlots,
	...
	FontSelectionSummary {
		...
		for (var entry : widths.int2ObjectEntrySet()) entry.setValue(IntLists.unmodifiable(entry.getValue()));
	...
	static long retainedWidthsBytes(Int2ObjectMap<IntList> widths) {
		...
		for (IntList bucket : widths.values()) bytes += 112L + aligned(24L + 4L * bucket.size());
		return bytes;
	}
