...
public final class FontPreparationBundle implements AutoCloseable {
	...
	private Map<Identifier, FontPreparedSelection> selections;
	...
	public Map<Identifier, FontPreparedSelection> selections() { return selections; }
	...
	@Override public void close() {
		Map<Identifier, FontPreparedSelection> byId;
		Map<FontProviderStackKey, FontPreparedSelection> byStack;
		synchronized (this) {
			if (closed) return;
			closed = true;
			byId = selections; byStack = selectionsByStack;
			selections = Map.of(); selectionsByStack = Map.of();
		}
		try {
	...
	FontPreparationBundle(Set<FontOption> options, Map<Identifier, FontPreparedSelection> selections,
		Map<FontProviderStackKey, FontPreparedSelection> selectionsByStack, FontReloadDiagnostics.Snapshot diagnostics,
		...
		for (FontPreparedSelection selection : this.selectionsByStack.values()) {
			...
			for (int index = 0; index < expected.size(); index++) {
				if (expected.get(index).provider() != providers.get(index).provider()
					|| expected.get(index).filter() != providers.get(index).filter()) { same = false; break; }
			}
