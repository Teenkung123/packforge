...
function Invoke-SelfTests($Catalog, $Registry) {
    ...
    Assert-MutationRejected 'non-available-pins-cannot-be-partial' { param($c) ($c.profiles | Where-Object id -eq 'fabric-quick-pack-default-on').externalMods[0] | Add-Member -NotePropertyName coordinate -NotePropertyValue 'modrinth:pSISfJ4O' } $Catalog $Registry
    ...
    Assert-MutationRejected 'successful-result-requires-evidence' { param($c) $p = $c.profiles | Where-Object id -eq 'fabric-sodium'; $p.result = $p.expectedPath; $p.PSObject.Properties.Remove('evidence') } $Catalog $Registry
